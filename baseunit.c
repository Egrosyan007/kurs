...
char *FillString(char *S, unsigned char len, unsigned char pk)
{
	...
	if (pk==0) {strcat(St,S); strcpy(S,St);}
	return S;
} //-----FillString() 
//------------------------------------------GetNumber()
                   ...
                   char m1,char n1,char m2,char n2)
{
  ...
  do
    ...
    if ((Number<MinNumber) || (Number>MaxNumber)) 
		{		//число выходит за указанный диапазон
  ...
  *Beg = NULL;
  ...
  while (fread(&Product,sizeof(ProductType),1,fArBin)==1)
	{	//выделение памяти для элемента дека
		...
		Run->Next=*Beg;		//связывание указателей
	  	*Beg=Run;
	}
	fclose(fArBin);
	ReversProduct(Beg); // реверс дека
	return 0;
}	//-----ReadFileOut() 
//-----------------------------------------DisposeProduct()
...
{           
	...
	fclose(fArBin);
	DisposeProduct(Beg);	//удаление стека
	return 0;
}	//-----WriteFileOut() 
//-----------------------------------------------
