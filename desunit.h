...
typedef struct ProductT // тип компонента архива изделий 
	{	
		char Names[20],							// ФИО призывника
			 Date[20],							// Дата рождения
			 Num[20],							// Номер личного дела
			 Street[35],						// Адрес проживания
			 IsApproved[15];					// Пригодность к службе
		
		unsigned	char	NumberShop;			// номер цеха 
							int		Kod ;   	// код изделия 
							char	Dimens[5] ; // единица измерения 
							double	Price ;  	// цена изделия 
							double  Plan[2],  	// план выпуска по полугодиям 
									Fact[2];   	// факт.выпуск по полугодиям 
